...
// Type for form data using zod schema
type AskQuestionFormData = z.infer<typeof AskQuestionSchema>;

const QuestionForm = () => {
  const editorRef = useRef<MDXEditorMethods>(null);

  // Use form hook with types from the zod schema
  ...
  const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: { value: string[] }) => {
    if (e.key === 'Enter') {
      ...
      // Check if the tag is valid and not already in the list
      if (tagInput) {
  ...
  const handleCreateQuestion: SubmitHandler<AskQuestionFormData> = data => {
